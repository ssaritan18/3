...
export type FriendRequest = { id: string; from: string; email?: string; senderId?: string; note?: string; timestamp?: string };
...
export function FriendsProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (syncEnabled && token) {
      ...
      }, 60000); // Changed to 60s to prevent rate limiting
    }
  ...
  const sendRequest = async (email: string, note?: string) => {
    ...
    setRequests((prev) => [...prev, { id: uid(), from: email, email, note }]);
  ...
  const acceptRequest = async (id: string) => {
    ...
    if (req) {
      setFriends((prev) => [...prev, { id: uid(), name: req.from, email: req.email || req.from }]);
      setRequests((prev) => prev.filter((r) => r.id !== id));
    }
  ...
  const rejectRequest = async (id: string) => {
    ...
    if (syncEnabled && token) {
      try {
        ...
        throw new Error(`Failed to reject friend request: ${error.response?.data?.detail || error.message || 'Network error'}`);
      }
    }

    setRequests((prev) => prev.filter((r) => r.id !== id));
    console.log("✅ Friend request rejected locally");
  };

...
import { View, Text, StyleSheet, TouchableOpacity, Alert, ScrollView, Platform, DevSettings, Switch, Linking } from "react-native";
...
import { api, API_BASE_URL, getBackendBaseUrl } from "../../src/lib/api";
...
export default function ProfileScreen() {
  ...
  // UI State
  const [activeTab, setActiveTab] = useState<'overview' | 'achievements' | 'stats' | 'store' | 'completion' | 'neurodivergency' | 'journey'>('overview');
  ...
  // Load profile data from AsyncStorage when screen focuses
  ...
  const seedDemo = async () => {
    ...
    try {
      const res = await api.post("/api/dev/seed-demo");
    ...
    const fetchBackendData = async () => {
      if (syncEnabled && token) {
        try {
          ...
          // Fetch streak data
          const streakResponse = await fetch(`${API_BASE_URL}/user/streak`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
          ...
          // Fetch points data
          const pointsResponse = await fetch(`${API_BASE_URL}/user/points`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
      ...
      >
        ...
        >
          ...
          <TouchableOpacity 
            onPress={async () => {
              ...
              try {
                const url = `${backendBaseUrl || 'http://localhost:3000'}/delete-account.html`;
        ...
        <TouchableOpacity 
          onPress={async () => {
            ...
            const confirmed = (typeof window !== 'undefined' && window.confirm) ? window.confirm("Are you sure you want to sign out?") : true;

...
def _get_mongo_url() -> str:
    """Return the Mongo connection string with sensible fallbacks."""
    url = os.getenv("MONGO_URL") or os.getenv("MONGODB_URL") or ""
    url = url.strip()
    if url:
        return url

    # Fall back to a safe local default for development environments
    default_url = "mongodb://localhost:27017/adhders_social_club"
    logger.warning(
        "MONGO_URL not provided; falling back to local default %s",
        _sanitize_mongo_url(default_url),
    )
    return default_url
...
REQUEST_LIMIT_PER_MINUTE = 100  # Allow 100 requests per minute per user (increased for production)
...
async def lifespan(app):
    ...
    sanitized_url = _sanitize_mongo_url(mongo_url)
   
    try:
        logger.info("Attempting MongoDB connection to %s", sanitized_url)
        ...
        if mongo_db_name:
            ...
            try:
                ...
                # Provide a deterministic default to avoid authentication issues due to empty DB context
                fallback_db_name = "adhders_social_club"
                logger.warning(
                    "Mongo connection string does not include a default database. "
                    "Falling back to '%s'. Consider setting MONGO_DB_NAME for clarity.",
                    fallback_db_name,
                )
        ...
        logger.info("✅ Connected to MongoDB successfully at %s", sanitized_url)
        yield
    except OperationFailure as exc:
        if exc.code == 8000:
            logger.error(
                "MongoDB authentication failed when connecting to %s. "
                "Please verify username, password, and allowed IP addresses.",
                sanitized_url,
            )
            raise RuntimeError(
                "MongoDB authentication failed; please verify credentials"
            ) from exc
        logger.exception("MongoDB operation failure when connecting to %s", sanitized_url)
        raise RuntimeError("MongoDB connection failed due to an operation error") from exc
    except Exception as exc:
        logger.exception("Unexpected MongoDB error when connecting to %s", sanitized_url)
        raise RuntimeError("MongoDB connection failed; please review configuration") from exc
    finally:
        if client:
            client.close()
        client = None
...
async def get_current_user(authorization: str = Header(default=None)):
    ...
    # Normalize identifier fields so callers always receive both variants
    user_id_value = user.get("_id")
    if user_id_value is not None:
        user["_id"] = str(user_id_value)
...
async def accept_friend_request_legacy(payload: FriendAcceptReq, user=Depends(get_current_user)):
    logger.warning("/friends/accept is deprecated. Use /friends/accept/{request_id} instead.")
    return await _accept_friend_request_core(payload.request_id, user)


@api_router.post("/friends/reject")
async def reject_friend_request_legacy(payload: FriendRejectReq, user=Depends(get_current_user)):
    logger.warning("/friends/reject is deprecated. Use DELETE /friends/reject/{request_id} instead.")
    return await _reject_friend_request_core(payload.request_id, user)


@api_router.delete("/friends/reject/{request_id}")
async def reject_friend_request_v2(request_id: str, user=Depends(get_current_user)):
    try:
        return await _reject_friend_request_core(request_id, user)
    except HTTPException:
        raise
    except Exception as e:
...
async def open_direct_chat(friend_id: str, user=Depends(get_current_user)):
    ...
    # Check if they are friends via friendships collection
...
ALLOW_LAN_ORIGINS = os.getenv("ALLOW_LAN_ORIGINS", "").lower() in {"1", "true", "yes", "on"}
lan_origin_regex = (
    r"http://(?:(?:10|127)\.\d{1,3}\.\d{1,3}\.\d{1,3}"
...
async def _normalize_friend_request_ids(friend_request: dict) -> tuple[str, str]:
    """Return canonical sender/recipient ids for a friend request document."""
    sender_id = friend_request.get("sender_id") or friend_request.get("from_user_id")
    recipient_id = friend_request.get("recipient_id") or friend_request.get("to_user_id")
    if not sender_id or not recipient_id:
        raise HTTPException(status_code=400, detail="Friend request is missing sender/recipient information")
    return sender_id, recipient_id


async def _ensure_friendship(sender_id: str, recipient_id: str) -> dict:
    """Create or fetch the friendship document for two users."""
    friendship = await db.friendships.find_one({
        "$or": [
...
async def _ensure_direct_chat(sender: dict, recipient: dict) -> dict:
    """Ensure a direct chat exists between two friends."""
    ...
    chat_doc = {
        "_id": chat_id,
        "type": "direct",
        "title": f"{sender.get('name', 'Friend')} & {recipient.get('name', 'Friend')}",
        "members": participants,
        "invite_code": None,
...
async def _accept_friend_request_core(request_id: str, recipient: dict) -> dict:
    friend_request = await db.friend_requests.find_one({"_id": request_id})
    if not friend_request:
        raise HTTPException(status_code=404, detail="Friend request not found")

    sender_id, recipient_id = await _normalize_friend_request_ids(friend_request)
    if recipient_id != recipient["_id"]:
        raise HTTPException(status_code=404, detail="Friend request not found")
    ...
    friendship = await _ensure_friendship(sender_id, recipient_id)
    ...
    await db.users.update_one({"_id": recipient_id}, {"$addToSet": {"friends": sender_id}})
    await db.users.update_one({"_id": sender_id}, {"$addToSet": {"friends": recipient_id}})

    chat = await _ensure_direct_chat(sender, recipient)

    now = now_iso()
    sender_payload = {
        "friend_id": recipient["_id"],
        "friend_name": recipient.get("name", recipient.get("email")),
        "friend_email": recipient.get("email"),
        "friendship_id": friendship["_id"],
        "timestamp": now,
    }
    recipient_payload = {
        "friend_id": sender_id,
        "friend_name": sender.get("name", sender.get("email")),
        "friend_email": sender.get("email"),
        "friendship_id": friendship["_id"],
        "timestamp": now,
    }

    await broadcast_to_user(sender_id, {
        "type": "friendRequestAccepted",
        "data": sender_payload,
    })
    await broadcast_to_user(recipient_id, {
        "type": "friendListUpdate",
        "data": recipient_payload,
    })

    await ws_broadcast_to_user(sender_id, {
    ...
    await ws_broadcast_to_friends(recipient_id, {"type": "friends:list:update"})
    await ws_broadcast_to_friends(sender_id, {"type": "friends:list:update"})

    logger.info(
        f"✅ Friend request accepted: {sender.get('name', sender_id)} ↔ {recipient.get('name', recipient_id)}"
    )
...
async def _reject_friend_request_core(request_id: str, recipient: dict) -> dict:
    friend_request = await db.friend_requests.find_one({"_id": request_id})
    if not friend_request:
        raise HTTPException(status_code=404, detail="Friend request not found")
    ...
    logger.info(
        f"🚫 Friend request rejected: {sender_id} → {recipient.get('name', recipient_id)}"
    )
...
async def accept_friend_request(request_id: str, user=Depends(get_current_user)):
    ...
    try:
        return await _accept_friend_request_core(request_id, user)
    except HTTPException:
        raise
    except Exception as e:
...
async def get_friends_list(user=Depends(get_current_user)):
    ...
    try:
        ...
        for friendship in friendships:
            ...
            if friend:
                friends.append({
                    "_id": friend_id,  # Use _id for consistency with frontend
                    "friend_id": friend_id,
                    "name": friend.get("name", friend["email"]),
...
async def like_community_post(post_id: str, current_user = Depends(get_current_user)):
    ...
    try:
        ...
        liked = False
        if user_like:
            delete_result = await db.community_likes.delete_one({"_id": user_like["_id"]})
            liked = False if delete_result.deleted_count else True
        else:
            like_doc = {
                "id": str(uuid.uuid4()),
                "post_id": post_id,
            ...
            insert_result = await db.community_likes.insert_one(like_doc)
            if not insert_result.inserted_id:
                raise HTTPException(status_code=500, detail="Failed to register like")
            liked = True

        likes_count = await db.community_likes.count_documents({"post_id": post_id})
        ...
        return {"success": True, "liked": liked, "likes": likes_count}
    except Exception as e:
...
async def share_community_post(post_id: str, current_user = Depends(get_current_user)):
    ...
    try:
        ...
        insert_result = await db.community_shares.insert_one(share_doc)
        if not insert_result.inserted_id:
            raise HTTPException(status_code=500, detail="Failed to record share")

        share_count = await db.community_shares.count_documents({"post_id": post_id})
        await db.community_posts.update_one(
            {"id": post_id},
            {"$set": {"shares": share_count}}
        )

        logger.info(f"✅ Community post {post_id} shared by {current_user['name']}")
        return {"success": True, "shared": True, "shares": share_count}
    except Exception as e:
...
async def reply_to_community_post(post_id: str, request: Request, current_user = Depends(get_current_user)):
    ...
    try:
        ...
        replies_count = await db.community_replies.count_documents({"post_id": post_id})
        await db.community_posts.update_one(
            {"id": post_id},
            {"$set": {"replies": replies_count}}
        )
        
        logger.info(f"✅ Reply created for post {post_id} by {current_user['name']}")
        return {"success": True, "reply": reply_dict, "replies": replies_count}
    except Exception as e:
...
async def delete_community_post(post_id: str, current_user = Depends(get_current_user)):
    ...
    try:
        ...
        # Delete the post
        delete_result = await db.community_posts.delete_one({"id": post_id})
...
async def privacy_policy():
    ...
    return {
        ...
        <html>
        <head>
            <title>Privacy Policy - ADHDers Social Club</title>
            <meta charset="utf-8">
        ...
        <body>
            <div class="container">
                ...
                <p>We collect information you provide directly to us, such as when you create an account, complete assessments, or use our features.</p>
                
                <h2>2. How We Use Your Information</h2>
                <p>We use your information to provide, maintain, and improve our services, including personalized ADHD assessments and community features.</p>
                ...
                <h2>5. Contact Us</h2>
                <p>If you have any questions about this Privacy Policy, please contact us at privacy@adhders.com</p>
            </div>
        </body>
        </html>
        """
...
async def terms_of_service():
    ...
    return {
        ...
        <html>
        <head>
            <title>Terms of Service - ADHDers Social Club</title>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1">
            <style>
                body { font-family: Arial, sans-serif; line-height: 1.6; margin: 40px; color: #333; }
        ...
        <body>
            <div class="container">
                ...
                <p>By accessing and using ADHDers Social Club, you accept and agree to be bound by the terms and provision of this agreement.</p>
                
                <h2>2. Use License</h2>
                <p>Permission is granted to temporarily download one copy of ADHDers Social Club for personal, non-commercial transitory viewing only.</p>
                ...
                <h2>5. Disclaimer</h2>
                <p>The information on this app is provided on an 'as is' basis. To the fullest extent permitted by law, this Company excludes all representations, warranties, conditions and terms.</p>
                
                <h2>6. Contact Information</h2>
                <p>If you have any questions about these Terms of Service, please contact us at legal@adhders.com</p>
            </div>
        </body>
        </html>
        """
